...
int main(){
  vector<string> file_names = Tokenize(execute("ls txt/variables/ 2> /dev/null"), "\n");
...
void WriteBaseHeader(const set<Variable> &all_vars,
  ...
  ofstream file("inc/baby_base.hh");
  ...
  for(size_t i = 0; i < names.size(); ++i){
    file << "#include \"baby_" << names.at(i) << ".hh\"\n";
  }
  file <<'\n';
...
void WriteSepHeader(const pair<string, set<Variable> > &sep_vars){
  ...
  ofstream file(("inc/baby_"+name+".hh").c_str());
  ...
  file << "  baby_" << name << "(const TString &filename); // Constructor to read tree\n\n";
...
void WriteSepSource(const pair<string, set<Variable> > &sep_vars){
  ...
  file << "baby_" << name << "::baby_" << name << "(const TString &filename):\n";
  if(vars.size()){
    const set<Variable>::const_iterator vars_end_2 = --vars.end();
